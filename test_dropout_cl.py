...
from config import ConfigDropoutCLAE
from utils import ensure_dir
from trainer import TrainerDropoutCLAE
import torch
import numpy as np
import os
import h5py
from cadlib.macro import EOS_IDX, EXT_IDX


def main():
    # create experiment cfg containing all hyperparameters
    cfg = ConfigDropoutCLAE('test')
...
def reconstruct(cfg):
    ...
    tr_agent = TrainerDropoutCLAE(cfg)
    ...
    for i, data in enumerate(pbar):
        ...
        with torch.no_grad():
            ...
            batch_out_vec = tr_agent.logits2vec(outputs, use_mask=cfg.use_ext_mask, force_close_loop=cfg.force_close_loop)
