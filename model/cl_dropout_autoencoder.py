...
class CADContrastiveDropoutTransformer(nn.Module):
    def __init__(self, cfg):
        super(CADContrastiveDropoutTransformer, self).__init__()
    ...
    def forward(
        ...
        commands_enc1_, args_enc1_ = _make_seq_first(commands_enc1, args_enc1)  # Possibly None, None

        if z is None:
            _z = self.encoder(commands_enc1_, args_enc1_)
            # _z = self.bottleneck(_z)  # bottleneck (Linear layer) 제거
